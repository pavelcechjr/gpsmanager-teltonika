@extends('layouts.app', ['title' => 'Skupiny zařízení'])
@section('header', 'Skupiny zařízení')
@section('header-actions')
    <x-btn :href="route('zarizeni.skupiny.create')" icon="plus">Nová skupina</x-btn>
@endsection

@section('content')
    @if ($groups->isEmpty())
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl">
            <x-empty-state icon="layers" title="Žádné skupiny" description="Skupiny pro Teltonika jednotky — vlastník, lokalita, použití." />
        </div>
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($groups as $g)
                <a href="{{ route('zarizeni.skupiny.edit', $g) }}"
                   class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-xs text-zinc-500 mt-2">{{ $g->devices_count }} zařízení</div>
                        </div>
                    </div>
                </a>
            @endforeach
