...
@section('header', $group ? "Skupina: {$group->name}" : 'Nová skupina zařízení')

@section('content')
    <form method="POST" action="{{ $group ? route('zarizeni.skupiny.update', $group) : route('zarizeni.skupiny.store') }}" class="max-w-3xl">
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Zařízení v této skupině</label>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto p-3 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg">
                    @forelse ($devices as $d)
                        <label class="flex items-center gap-2 text-sm cursor-pointer hover:bg-white dark:hover:bg-zinc-800 p-1.5 rounded">
                            <input type="checkbox" name="devices[]" value="{{ $d->id }}" @checked(in_array($d->id, $selected)) class="rounded">
                            <span class="font-mono text-xs">{{ $d->imei }}</span>
                            @if($d->model)<span class="text-zinc-500 text-xs">({{ $d->model }})</span>@endif
                        </label>
                    @empty
                        <div class="text-sm text-zinc-500 col-span-2">Žádná aktivní zařízení.</div>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between mt-6">
            <a href="{{ route('zarizeni.skupiny') }}" class="text-sm text-zinc-500"><i data-lucide="arrow-left" class="inline w-4 h-4"></i> Zpět</a>
            <div class="flex gap-2">
                @if ($group)
                    <form method="POST" action="{{ route('zarizeni.skupiny.destroy', $group) }}" onsubmit="return confirm('Smazat skupinu?');">
                        @csrf @method('DELETE')
